...
function FlexItem({ icon, title, description }: FlexItemProps) {
  return (
    <div className="flex flex-col gap-0.5 flex-1 min-w-0 items-center text-center">
      <div className="relative w-9 h-9 md:w-24.5 md:h-24.5 shrink-0 ">
        <Image
          src={icon}
          alt={title}
          fill
          sizes="(min-width: 768px) 98px, 36px"
          className="object-contain"
        />
      </div>
      <h3 className="font-bold text-[14px] leading-4.5 md:text-[22px] md:leading-7 font-work-sans text-dark">
        {title}
      </h3>
      <p className="text-[12.5px] font-manrope leading-4 md:text-[18px] md:leading-7 text-secondary">
        {description}
      </p>
    </div>
  );
